...
vec3 AreaLight::GetRadiance(const SceneSphere& /*aSceneSphere*/,
                            ...
                            float*      emissionPdfW_ /*= nullptr*/) const
{
    ...
    {
        *emissionPdfW_ =
            sampleHemisphereCosinePdfW(m_frame.getNormal(), -_rayDirection);
...
vec3 AreaLight::Emit(const SceneSphere& /*aSceneSphere*/,
                     ...
                     float*      cosThetaLight_) const
{
    ...
    vec3 localDirOut =
        sampleHemisphereCosineW(_dirRndTuple.x, _dirRndTuple.y, &emissionPdfW_);
...
vec3 AreaLight::Illuminate(const SceneSphere& /*aSceneSphere*/,
...
vec3 DirectionalLight::GetRadiance(const SceneSphere& /*aSceneSphere*/,
...
vec3 DirectionalLight::Emit(const SceneSphere& _sceneSphere,
                            ...
                            float*      cosThetaLight_) const
{
    ...
    position_ =
        _sceneSphere.m_cenetr +
        _sceneSphere.m_radius * m_frame.toWorld(vec3(xy.x, xy.y, -1.0f));
...
vec3 DirectionalLight::Illuminate(const SceneSphere& _sceneSphere,
                                  ...
                                  float* cosAtLight_ /*= nullptr*/) const
{
    ...
    {
        *emissionPdfW_ =
            sampleConcentricDiscPdfA() * _sceneSphere.m_inv_radius_sqr;
    }
...
vec3 PointLight::GetRadiance(const SceneSphere& /*aSceneSphere*/,
                             const vec3& /*aRayDirection*/,
                             const vec3& /*aHitPoint*/,
                             float* directPdfA_ /*= nullptr*/,
                             float* emissionPdfW_ /*= nullptr*/) const
...
vec3 PointLight::Emit(const SceneSphere& /*aSceneSphere*/,
                      const vec2& _dirRndTuple,
                      ...
                      float* oCosThetaLight) const
{
    ...
    direction_ =
        sampleSphereUniform(_dirRndTuple.x, _dirRndTuple.y, &emissionPdfW_);
...
vec3 PointLight::Illuminate(const SceneSphere& /*aSceneSphere*/,
                            const vec3& _receivingPosition,
                            const vec2& _rndTuple,
                            vec3&       directionToLight_,
                            float&      distance_,
...
vec3 BackgroundLight::GetRadiance(const SceneSphere& _sceneSphere,
                                  const vec3& /*aRayDirection*/,
                                  const vec3& /*aHitPoint*/,
                                  float* directPdfA_ /*= nullptr*/,
                                  float* emissionPdfW_ /*= nullptr*/) const
...
vec3 BackgroundLight::Emit(const SceneSphere& _sceneSphere,
                           const vec2&        _dirRndTuple,
                           const vec2&        _posRndTuple,
                           vec3&              position_,
                           vec3&              direction_,
                           ...
                           float*             cosThetaLight_) const
{
    ...
    direction_ =
        sampleSphereUniform(_dirRndTuple.x, _dirRndTuple.y, &directPdf);
    ...
    position_ = _sceneSphere.m_cenetr +
                _sceneSphere.m_radius * frame.toWorld(vec3(xy.x, xy.y, -1.0f));

    // oPosition = Vec3f(-1.109054f, -2.15064538f, -1.087019148f);

    emissionPdfW_ =
        directPdf * sampleConcentricDiscPdfA() * _sceneSphere.m_inv_radius_sqr;
...
vec3 BackgroundLight::Illuminate(const SceneSphere& _sceneSphere,
                                 const vec3&        _receivingPosition,
                                 const vec2&        _rndTuple,
                                 vec3&              directionToLight_,
                                 float&             distance_,
                                 ...
                                 float* cosAtLight_ /*= nullptr*/) const
{
    // Replace these two lines with image sampling
    directionToLight_ =
        sampleSphereUniform(_rndTuple.x, _rndTuple.y, &directPdfW_);
    ...
    if (emissionPdfW_)
    {
        *emissionPdfW_ = directPdfW_ * sampleConcentricDiscPdfA() *
...
vec3 SphericalLight::GetRadiance(const SceneSphere& /*aSceneSphere*/,
                                 const vec3& _rayDirection,
                                 const vec3& _hitPoint,
                                 float*      directPdfA_ /*= nullptr*/,
                                 float*      emissionPdfW_ /*= nullptr*/) const
...
vec3 SphericalLight::Emit(const SceneSphere& /*aSceneSphere*/,
                          const vec2& _dirRndTuple,
                          const vec2& _posRndTuple,
                          vec3&       position_,
                          vec3&       direction_,
...
{
    position_ =
        m_pos + m_radius * sampleSphereUniform(_posRndTuple.x, _posRndTuple.y);
...
vec3 SphericalLight::Illuminate(const SceneSphere& /*aSceneSphere*/,
                                const vec3& _receivingPosition,
                                const vec2& _rndTuple,
                                vec3&       directionToLight_,
                                float&      distance_,
                                ...
                                float*      cosAtLight_ /*= nullptr*/) const
{
    const vec3 lightPoint =
        m_pos + m_radius * sampleSphereUniform(_rndTuple.x, _rndTuple.y);
    ...
    if (cosAtLight_) *cosAtLight_ = cosNormalDir;

    if (emissionPdfW_) *emissionPdfW_ = m_invArea * cosNormalDir / NUM_PI;

...
{
    return ::luminance(vec3(r, g, b));
}

// DirLight EnvmapLoader::getLightRandom() {
//     return m_lights[clampi(randf() * m_lights.size(), 0, m_lights.size() -
//     1)];
// }
...
void EnvmapLoader::analyze()
{
    ...
    if (0)
    {
        ...
        for (int n = 0; n < 100000; n++)
        {
            ...
            int u_i = static_cast<int>(u * m_header.width);
            int v_i = static_cast<int>(v * m_header.height);

            u_i = clampi(u_i, 0, m_header.width - 1);
            v_i = clampi(v_i, 0, m_header.height - 1);

            int offset = (u_i + v_i * m_header.width) * 3;
            m_buffer[offset + 0] += 0.1f;
            m_buffer[offset + 1] += 0.1f;
            m_buffer[offset + 2] += 0.1f;
        }
...
void EnvmapLoader::scale(float x)
{
    ...
    {
        lum[i / 3] =
            luminance(m_buffer[i + 0], m_buffer[i + 1], m_buffer[i + 2]);
    }
    std::cout << "envmap max = " << *std::max_element(lum.begin(), lum.end())
              << std::endl;
}
...
// the whole algorithm is written compactly here to implement the corresponding
// matlab code test_marginal_pdf.m
{
    ...
    if (is_pre_warped)
    {
        for (int m = 0; m < M; m++)
        {
            ...
            for (int n = 0; n < N; n++)
            {
                lum[n + m * N] *= sin(phi);  // should be M_PI - phi, but does
                                             // not matter due to symmetry
            }
        }
    ...
    m_normalizationFactor =
        1.0f /
        std::accumulate(lum.begin(),
                        lum.end(),
                        0.0f);  // MUST use 0.0 or 0.0f instead of 0 !!!!!!!!!!
    //////////////////////////////////////////////////////////////////////////
    ...
    //     float m_pdf_u_norm = 1.0f / std::accumulate(m_pdf_u.begin(),
    //     m_pdf_u.end(), 0.0f); std::transform(m_pdf_u.begin(), m_pdf_u.end(),
    //     m_pdf_u.begin(), multiply_by(m_pdf_u_norm));
    //
    //     // prepare cdf_u
    //     float acc = 0;
    //     for (int n = 0; n < N; n++) {
    ...
    //             sum2 += m_normalizationFactor * m_luminance[n + m * N] *
    //             float(m_header.width) * float(m_header.height);
    //
    //             //             printf("pdf1 = %f\n", lum[n + m * N] *
    //             m_normalizationFactor);
    //             //             printf("pdf2 = %f\n", m_pdf_u[n] * m_pdf_uv[n
    //             + m * N]);
    //         }
    //     }
    //     printf("sum = %f\n", sum);
    ...
    {
        column_sum[n] =
            std::accumulate(columns[n].begin(), columns[n].end(), 0.0f);
    }
...
void EnvmapLoader::sampleByPdf(float  rnd_u,
                               float  rnd_v,
                               float& u_ref,
                               float& v_ref) const
{
...
DirLight EnvmapLoader::sample(float r1, float r2) const
{
    ...
    int u_i =
        clampi(static_cast<int>(u * m_header.width), 0, m_header.width - 1);
    int v_i =
        clampi(static_cast<int>(v * m_header.height), 0, m_header.height - 1);
    ...
    dl.pdf = m_normalizationFactor * m_luminance[u_i + v_i * m_header.width] *
             float(m_header.width) * float(m_header.height) /
             (2 * NUM_PI * NUM_PI /* * sin(phi) */ /* this one is cancelled */);
...
float EnvmapLoader::getPdf(float dir[3]) const
{
    ...
    int   u_i = clampi(
        int(theta / (NUM_PI * 2) * m_header.width), 0, m_header.width - 1);
    int v_i =
        clampi(int(phi / NUM_PI * m_header.height), 0, m_header.height - 1);

    float pdf =
        m_normalizationFactor * m_luminance[u_i + v_i * m_header.width] *
        float(m_header.width) * float(m_header.height) /
        (2 * NUM_PI * NUM_PI /* * sin(phi) */ /* this one is cancelled */);

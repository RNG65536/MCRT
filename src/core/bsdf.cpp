...
float AbstractBSDF::getContinuationProbability(const vec3& wi,
                                               const vec3& nl) const
{
...
vec3 LambertianBSDF::sample(const vec3& wi,
                            const vec3& nl,
                            float       r1,
                            float       r2,
                            vec3&       total_weight,
                            ...
                            vec3*       bsdf_weight) const
{
    ...
    {
        *forward_pdfW =
            _cos / NUM_PI;  // wrt solid angle, i.e., dw is the integrand
        // *forward_pdfW = 1.0 / NUM_PI; // wrt projected solid angle, i.e.
        // cos*dw is the integrand
    }
    if (cos_wo)
    {
        *cos_wo = _cos;
...
float LambertianBSDF::calculatePdfW(const vec3& wi,
                                    const vec3& nl,
                                    const vec3& wo) const
{
    return debug_f_max(dot(wo, nl), NUM_EPS_COSINE) /
           NUM_PI;  // not the same if wi and wo are swapped ??
}

vec3 LambertianBSDF::evaluate(const vec3& wi,
                              const vec3& nl,
                              const vec3& wo,
                              float*      cos_wo,
...
vec3 SpecularBSDF::sample(const vec3& wi,
                          const vec3& nl,
                          float       r1,
                          float       r2,
                          vec3&       total_weight,
                          ...
                          vec3*       bsdf_weight) const
{
    ...
    {
        *forward_pdfW =
            1;  // wrt projected solid angle, otherwise needs to divide cos_wo
    }
    if (bsdf_weight)
    {
...
float SpecularBSDF::calculatePdfW(const vec3& wi,
                                  const vec3& nl,
                                  const vec3& wo) const
{
    return 0;
...
vec3 SpecularBSDF::evaluate(const vec3& wi,
                            const vec3& nl,
                            const vec3& wo,
                            float*      cos_wo,
                            float*      forward_pdfW /*= nullptr*/) const
...
vec3 GlossyBSDF::sample(const vec3& wi,
                        const vec3& nl,
                        float       r1,
                        float       r2,
                        vec3&       total_weight,
                        ...
                        vec3*       bsdf_weight) const
{
    ...
    {
        // wrt solid angle
        *forward_pdfW = (g + 1.0f) / (2.0f * NUM_PI) *
                        pow(debug_f_max(dot(refl, wo), NUM_EPS_COSINE), g);
        // wrt projected solid angle
        //         *forward_pdfW = (g + 1.0) / (2.0 * NUM_PI) *
        //         pow(f_max(dot(refl, wo), 0.0), g) / _cos;
    }

    float wo_n = dot(wo, nl);
    float wi_n = dot(wi, nl);
    float weight =
        (g + 2.0f) / (g + 1.0f) * _cos / debug_f_max(fabs(wi_n), fabs(wo_n));
    ...
    if (bsdf_weight)
    {
        *bsdf_weight = ((g + 2.0f) / (2.0f * NUM_PI) *
                        pow(debug_f_max(dot(refl, wo), NUM_EPS_COSINE), g) /
    ...
    total_weight =
        weight * parent->color();  // bsdf_weight * cos_wo / forward_pdfW
    return wo;
}

float GlossyBSDF::calculatePdfW(const vec3& wi,
                                const vec3& nl,
                                const vec3& wo) const
{
    float g = parent->glossiness();
    vec3  refl = reflectWorld(wi, nl);
    return (g + 1.0f) / (2.0f * NUM_PI) *
           pow(debug_f_max(dot(refl, wo), NUM_EPS_COSINE), g);
}

vec3 GlossyBSDF::evaluate(const vec3& wi,
                          const vec3& nl,
                          const vec3& wo,
                          float*      cos_wo,
                          float*      forward_pdfW /*= nullptr*/) const
...
vec3 RefractiveBSDF::sample(const vec3& wi,
                            const vec3& nl,
                            float       r1,
                            float       r2,
                            vec3&       total_weight,
                            ...
                            vec3*       bsdf_weight) const
{
    ...
    vec3 refl_dir =
        normalize(reflectWorld(wi, n));  // Ideal dielectric REFRACTION
    float nc = 1, nt = parent->ior();

    // nnt is reciprocal relative index of refraction, ddn is negated cosine of
    // incidence angle
...
#else
        float Re = fresnelReflectance(
            dot(wi, nl),
            nt / nc);  // Schlick's approximation for Fresnel reflectance
#endif
        float Tr = 1 - Re;
...
float RefractiveBSDF::calculatePdfW(const vec3& wi,
                                    const vec3& nl,
                                    const vec3& wo) const
{
    return 0;
...
vec3 RefractiveBSDF::evaluate(const vec3& wi,
                              const vec3& nl,
                              const vec3& wo,
                              float*      cos_wo,
                              float*      forward_pdfW /*= nullptr*/) const

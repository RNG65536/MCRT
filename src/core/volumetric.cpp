...
{
    //float s=2.0*e1-1.0, f = (1.0-g*g)/(1.0+g*s), cost = 0.5*(1.0/g)*(1.0+g*g-f*f), sint = std::sqrtf(1.0-cost*cost);
    float s = 1.0f - 2.0f*e1, denom = 1.0f + g*s;
    float cost = (s + 2.0f*g*g*g*(-1.0f + e1) * e1 + g*g*s + 2.0f*g*(1.0f - e1 + e1*e1)) / (denom * denom), sint = std::sqrtf(1.0f - cost*cost);
    return vec3(cosf(2.0f * NUM_PI * e2) * sint, sinf(2.0f * NUM_PI * e2) * sint, cost);
}
...
float scatter(const Ray &r, Ray &sRay, float sigma_s, float &s, float e0, float e1, float e2)
{
    ...
    vec3 x = r.o + r.d * s;
    //     Vector3 dir = sampleHG(0.9f, e1, e2); //Sample a direction ~ Henyey-Greenstein's phase function
    vec3 dir = sampleSphereUniform(e1, e2); //Sample a direction ~ uniform phase function

//     vec3 u, v;
//     generateOrthoBasis(u, v, r.d);
//     dir = u * dir.x + v * dir.y + r.d * dir.z;
    OrthonormalFrame onb(r.d);
    dir = onb.toWorld(dir);
...
scale(30)
{
    ...
    printf("sigma_a:<%f, %f, %f>\n",
        final_sigma_a.x, final_sigma_a.y, final_sigma_a.z);
    printf("sigma_s:<%f, %f, %f>\n",
        final_sigma_s.x, final_sigma_s.y, final_sigma_s.z);
    printf("sigma_t:<%f, %f, %f>\n",
        final_sigma_t.x, final_sigma_t.y, final_sigma_t.z);
    printf("albedo<%f, %f, %f>\n",

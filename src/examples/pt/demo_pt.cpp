...
static void loadSceneArmadillo(Scene& scene, FrameBuffer& film)
{
    ...
    //         vec3(1,1,1),   30,       DIFF);
    //     add_CUDA(BoxObject(vec3(0,0,3)+world_shift,vec3(1,1,4)+world_shift),
    //     vec3(1,1,1),   0,       REFR);
    //     add_CUDA(BoxObject(vec3(0,0,3)+world_shift,vec3(1,1,4)+world_shift),
    //     vec3(1,1,1),   0,       DIFF);
    ...
    scene.loadModel("bunny_1k.obj", shift, Material(vec3(10, 10, 10), 0, LGHT));
    ...
    //             obj.vertexArray[n] =
    //             sl.onPosition(ry.onPosition(rx.onPosition(obj.vertexArray[n])));
    //         }
    //         obj.genNorm();
    //     }
    ...
    scene.loadModel(
        "Armadillo.obj", scaling * rotation, Material(vec3(1, 1, 1), 0, DIFF));

    // camera
    float zTrans = 7;
    float yTrans = 2.5;
    vec3  lookat(scene.boxCenter());
    vec3  rayorig = lookat + vec3(0, yTrans * 0.8f, zTrans * 0.5f) -
                   vec3(0, 2.1f, 0);  // no x-shift
    float viewdist = (lookat - rayorig).length();
    float _rot = -120 / 180.0f * NUM_PI;
    rayorig = vec3(-viewdist * sin(_rot), yTrans * 0.5f, viewdist * cos(_rot)) +
              lookat;
    {
...
static void loadSceneCornellBox(Scene& scene, FrameBuffer& film)
{
    ...
    //////////////////////////////////////////////////////////////////////////
#if 1
    vec3 cbox_luminaire[] = {vec3(343, 548.79999f - 0.5f, 227),
                             vec3(343, 548.79999f - 0.5f, 332),
                             vec3(213, 548.79999f - 0.5f, 332),
                             vec3(213, 548.79999f - 0.5f, 227)};

    vec3 cbox_luminaire2[] = {vec3(343, 548.79999f - 0.5f, 227),
                              vec3(343, 548.79999f - 0.5f, 332),
                              vec3(213, 548.79999f - 0.5f, 332),
                              vec3(213, 548.79999f - 0.5f, 227),
    ...
    //////////////////////////////////////////////////////////////////////////
    vec3 cbox_greenwall[] = {vec3(0, 0, 559.20001f),
                             vec3(0, 0, 0),
                             vec3(0, 548.79999f, 0),
    ...
    //////////////////////////////////////////////////////////////////////////
    vec3 cbox_back[] = {vec3(556, 0, 559.20001f),
                        vec3(0, 0, 559.20001f),
                        vec3(0, 548.79999f, 559.20001f),
                        vec3(556, 548.79999f, 559.20001f)};
    ...
    //////////////////////////////////////////////////////////////////////////
    vec3 cbox_ceiling[] = {vec3(556, 548.79999f, 0),
                           vec3(556, 548.79999f, 559.20001f),
                           vec3(0, 548.79999f, 559.20001f),
                           vec3(0, 548.79999f, 0)};
...
public:
    ...
    vec3 sampleEnvmapUsingLightPDF(const vec3&     wi,
                                   const vec3&     pos,
                                   const vec3&     nl,
                                   float&          pdf,
                                   vec3&           dir,
                                   const Material& bsdf)
    {
        ...
        {
            float g = f_max(
                0, dot(lightDirection, nl));  // cosine factor for irradiance
            if (g > 0)
            {
                // float weightEnvmap = pscene->envmap()->getPdf(dl.dir) /
                // (pscene->envmap()->getPdf(dl.dir) + g / M_PI);
                pdf = pscene->envmap()->getPdf(dl.dir);
                dir = lightDirection;
                irr += (g * bsdf.evaluate(wi, nl, lightDirection) / dl.pdf) *
                       vec3(dl.rad[0],
                            dl.rad[1],
                            dl.rad[2]);  // note the 1/M_PI in the BRDF!!!!!
            }
        }
        if (irr != irr)
        {
    ...
    vec3 sampleEnvmapUsingSurfacePDF(const vec3&     wi,
                                     const vec3&     pos,
                                     const vec3&     nl,
                                     float&          pdf,
                                     vec3&           dir,
                                     const Material& bsdf)
    {
        ...
        // TODO: use utility
        vec3 w = nl, u = normalize(cross(vec3(0.3f, 0.4f, 0.5f), w)),
             v = cross(w, u);
        vec3 wo =
            (cos(r1) * r2s * u + sin(r1) * r2s * v + std::sqrt(1 - r2) * w)
                .normalize();
        ...
        if (!occluded)
        {
            ...
            if (g > 0)
            {
                // float weightBRDF = (g / M_PI) /
                // (pscene->envmap()->getPdf(&d[0]) + g / M_PI);
                // pdf = g / M_PI;
...
{
    bool passive = 0;  // no direct light sampling
    ...
    for (int n = 0; n <= maxDepth; n++)
    {
        ...
        bool no_direct = passive || n == 0 || last_mat != DIFF;

        // passive envmap sampling
        if (!hit)
        {
            if (no_direct && scene.envmap())
            {
                vec3 r;
        ...
        // passive light sampling
        if (mat.type() == LGHT)
        {
            if (no_direct && dot(cr.dir, nhit) < 0)
            {
        ...
        if (!passive && (mat_type == DIFF))
        {
            if (scene.envmap())
            {
                ...
                // because bsdf.evaluate() already considers material color
                vec3 rad_envmap = rs.sampleEnvmapUsingLightPDF(
                    -cr.dir, phit, nhit, envmapPDF, envmapDir, mat);

                // multiply by color manually
                vec3 rad_bsdf = rs.sampleEnvmapUsingSurfacePDF(
                    -cr.dir, phit, nhit, bsdfPDF, bsdfDir, mat);
                rad_bsdf = mult(objColor, rad_bsdf);

                float weightEnvmap =
                    MIS_weight(envmapPDF, mat.pdfW(-cr.dir, nhit, envmapDir));
                float weightBSDF =
                    MIS_weight(bsdfPDF, scene.envmap()->getPdf(&bsdfDir[0]));

                vec3 rad = rad_envmap * weightEnvmap + rad_bsdf * weightBSDF;
                // vec3 rad = rad_envmap;
                // vec3 rad = rad_bsdf;

                local_radiance_out += rad;
            }

            if (scene.lightArea() > 0)
            {
                // BRDF sampling
                vec3  weight;
                float pdfW_BRDF;
                vec3  brdf_sampled_dir = mat.sample(
                    -cr.dir, nhit, randf(), randf(), weight, &pdfW_BRDF);

                float pdfW_light_passive =
                    scene.lightPdfW(phit, nhit, brdf_sampled_dir);
                float mis_weight_brdf =
                    MIS_weight(pdfW_BRDF, pdfW_light_passive);
                vec3 rad_brdf_dir =
                    scene.lightRadiance(phit, /*nhit,*/ brdf_sampled_dir);

                if (pdfW_light_passive == 0 &&
                    rad_brdf_dir.length() >
                        0)  // these are the cases that cause fireflies
                {
                    std::cout << "{ " << mis_weight_brdf << " }" << std::endl;
                }

                local_radiance_out +=
                    weight * (rad_brdf_dir) * (mis_weight_brdf);
                ...
                int   light_id;
                float pdfW_light;
                vec3  light_sampled_dir =
                    scene.sampleLightArea(phit, /*nhit,*/ pdfW_light, light_id);

                if (pdfW_light > 0)
                {
                    // light weighting
                    float pdfW_BRDF_passive =
                        mat.pdfW(-cr.dir, nhit, light_sampled_dir);
                    float mis_weight_light =
                        MIS_weight(pdfW_light, pdfW_BRDF_passive);

                    vec3 rad_light_dir = scene.lightRadiance(
                        phit, /*nhit,*/ light_sampled_dir, light_id);
                    vec3 weight =
                        mat.evaluate(-cr.dir, nhit, light_sampled_dir);
                    float cos_theta_light =
                        std::max(dot(nhit, light_sampled_dir), 0.0f);
                    local_radiance_out +=
                        weight * (rad_light_dir) *
                        (cos_theta_light / pdfW_light * mis_weight_light);
                }
            }
        }
...
#pragma omp parallel for schedule(dynamic)
        for (int x = 0; x < width; ++x)
        {
            ...
            for (int s = 0; s < spp; ++s)
            {
                ...
                Ray ray = cam->makeRay((x + 0.5f + dx) / width,
                                       (y + 0.5f + dy) / height);

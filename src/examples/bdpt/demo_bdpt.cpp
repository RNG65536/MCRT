...
static void addSphere(Scene& scene, const vec3& position, float radius, const vec3& color, MaterialType mat_type)
{
...
static void loadModel0(Scene& scene, FrameBuffer& film)
{
    ...
    float scale = 0.1;
    scene.setCamera(vec3(50.0, 40.8, 220.0) * scale, vec3(50.0, 40.8, 0.0) * scale, film.width(), film.height(), 40.0f);

    // light
    addSphere(scene, vec3(10, 70, 51.6) * scale, 6.0 * scale, vec3(100., 100., 100.), LGHT);
//     addSphere(scene, vec3(10, 70, 51.6) * scale, 10.0 * scale, vec3(1.0, 1.0, 1.0), LGHT);
...
#else
//     addSphere(scene, vec3(1e5 + 1 - 2e5, 40.8, 81.6) * scale, 1e5 * scale, vec3(0.75, 0.25, 0.25), DIFF);
//     addSphere(scene, vec3(-1e5 + 99 + 2e5, 40.8, 81.6) * scale, 1e5 * scale, vec3(0.25, 0.25, 0.75), DIFF);
//     addSphere(scene, vec3(50, 40.8, 1e5 - 2e5) * scale, 1e5 * scale, vec3(0.75, 0.65, 0.75), DIFF);
//     addSphere(scene, vec3(50, 40.8, -1e5 + 350 + 2e5) * scale, 1e5 * scale, vec3(0.50, 0.50, 0.50), DIFF);
    addSphere(scene, vec3(50, 1e5 - 2e5, 81.6) * scale, 1e5 * scale, vec3(0.65, 0.75, 0.75), DIFF); // floor
//     addSphere(scene, vec3(50, 1e5 - 2e5, 81.6) * scale, 1e5 * scale, vec3(1.0, 1.0, 1.0), DIFF); // floor
//     addSphere(scene, vec3(50, -1e5 + 81.6 + 2e5, 81.6) * scale, 1e5 * scale, vec3(0.75, 0.75, 0.65), DIFF);
#endif

    // objects
    addSphere(scene, vec3(50, 20, 50) * scale, 20 * scale, vec3(0.25, 0.75, 0.25), REFR);
    addSphere(scene, vec3(19, 16.5, 25) * scale, 16.5 * scale, vec3(0.99, 0.99, 0.99), SPEC);
//     addSphere(scene, vec3(50, 20, 50) * scale, 20 * scale, vec3(1.0, 1.0, 1.0), DIFF);
//     addSphere(scene, vec3(19, 16.5, 25) * scale, 16.5 * scale, vec3(1.0, 1.0, 1.0), SPEC);
//     addSphere(scene, vec3(77, 16.5, 78) * scale, 16.5 * scale, vec3(0.99, 0.99, 0.99), REFR);
...
static void loadModel1(Scene& scene, FrameBuffer& film)
{
    ...
    scene.setCamera(vec3(3.137931, 1.571292, -1.968698), vec3(0.105605, 0.321292, -0.217984), film.width(), film.height(), 40.0f);
...
//         }
        ...
        const Material& mat = todo_getMaterial(hit.getObject()->materialID());

        // now only light can emit radiance
        if (mat.type() == LGHT)
        {
            if (dot(cr.d, hit.getShadingNormal()) < 0)
            {
        ...
        vec3 p_hit = cr.at(hit.getDistance());
        vec3 n_hit = hit.getShadingNormal();
        // vec3 nhit_g = hit.getObject()->getFaceNormal();

        // continue tracing
        vec3 total_weight;
        vec3 wo = mat.sample(-cr.d, n_hit, randf(), randf(), total_weight);
...
#pragma omp parallel for schedule(dynamic)
        for (int x = 0; x < width; ++x)
        {
            ...
            for (int s = 0; s < spp; ++s)
            {
                ...
                Ray ray = scene.getCamera()->makeRay((x + 0.5 + dx) * invWidth, (y + 0.5 + dy) * invHeight);
...
{
    bdpt = new ReferenceBDPT(*scene.getCamera(), film, scene);

    WTimer tm;

    // integration
    unsigned int total_samples = film.width() * film.height() * 100;// 100;// *100;// *1000; // 100000;
    int step = total_samples / 100;

#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < total_samples; i++)
    {
        bdpt->render(MaxEvents);
    ...
    float brightness_scale = float(film.width()) * float(film.height()) / float(total_samples);

...
private:
    ...
    // path sampling (recursively construct a path, but the first vertex is always constructed outside this function)
    void TracePath(Path &path, const Ray& ray, const int RayLevel, const int MaxRayLevel, PathType path_type)
    {
        ...
        vec3 position = ray.at(hit.getDistance());
        vec3 object_normal = hit.getShadingNormal();
        vec3 orienting_normal = dot(object_normal, ray.d) < 0 ? object_normal : -object_normal;
        ...
        {
            total_pdfA *= camera.measurementFunction(position, orienting_normal);
        }
        else
        {
            const vec3& previous_position = last_vertex.p; // can also use ray.o
            float previous_pdfW = last_vertex.pWo;
            float pdfA = previous_pdfW * pdfWtoA(previous_position, position, orienting_normal);
            total_pdfA *= pdfA;

            float G = geometryTerm(previous_position, last_vertex.getOrientingNormal(),
                position, orienting_normal);
        ...
        const float rnd0 = randf();
        const float rnd1 = randf();
        ...
        nr.d = mat.sample(-ray.d, object_normal, rnd0, rnd1, total_weight, &pdfW, &cos_wo, &bsdf_weight);
        if (mat.isDelta())
        {
            bsdf_weight /= cos_wo;
        }
        ...
        orienting_normal = dot(object_normal, nr.d) > 0 ? object_normal : -object_normal;
        ...
        Vert vert(position, object_normal, obj);
        {
            ...
            vert.debug_vertex_type = mat.isLight() ? Emitter :
                (mat.isDelta() ? Specular : Diffuse);
        }
        ...
        if (! mat.isLight())
        {
            TracePath(path, nr, RayLevel + 1, MaxRayLevel, path_type);
        }
    }
    ...
    Path GenerateEyePath(const int MaxEyeEvents)
    {
        ...
        Ray r = camera.sample(randf(), randf());

        Vert v(r.o, camera.getForwardDirection(), nullptr);
        v.debug_type = FromEye;
        v.debug_wo = r.d;
        v.debug_vertex_type = Lens;
        v.setDeltaFlag(false);
        v.setTotalPdfA(1.0);
        v.setOrientingNormal(camera.getForwardDirection());
        v.setThroughput(vec3(1.0));
        v.weight = vec3(1);
        v.pWo = 1.0;
        Result.insert(v); // first vertex is on the camera aperture (if pinhole then it's the camera position)

        TracePath(Result, r, 1, MaxEyeEvents, FromEye);
        return Result;
    }
    ...
    Path GenerateLightPath(const int MaxLightEvents)
    {
        ...
        float pdfA, pdfW;
        scene.sampleLightAreaAndDirection(position, normal, pdfA, direction, pdfW, light_obj);
        ...
        v.setThroughput(todo_getMaterial(light_obj->materialID()).evaluate(vec3(), normal, direction));
        v.weight = vec3(1);
        v.pWo = pdfW;
        Result.insert(v); // first vertex is on the light's surface

        Ray r(position, direction);
        TracePath(Result, r, 1, MaxLightEvents, FromLight);
        return Result;
    }

    // check if the path can be connected or not (visibility term)
    // note that there would be false positives if eps is not set,
    // and this would lead to fireflies for bsdf may return a zero valued cosine term
    bool isConnectable(const Path& eye, const Path& light, float &px, float &py)
    {
        ...
        const Vert &ev = eye.end(); // the last vertex of eye path, to be connected
        const Vert &lv = light.end(); // the last vertex of light path, to be connected
        ...
        else if ((eye.size() >= 2) && (light.size() == 0))
        {
            ...
            // a temporary fix for the fireflies due to s=0 paths that have second last vertices too close to the light
            // TODO : see if s=0 paths are difficult for BDPT
//             be_visible = todo_getMaterial(ev.obj->materialID()).isLight() && ev.canConnectTo(eye[eye.size() - 2].p);
        ...
        else
        {
            ...
            Ray r(ev.p, d);
            be_visible = ev.canConnectTo(lv.p) && lv.canConnectTo(ev.p) && !scene.occluded(r, length(ev.p - lv.p));
            Direction = normalize(eye[1].p - eye[0].p); // primary ray direction
        }

        // get the pixel location
        return be_visible && camera.rasterizePrimaryRay(Direction, px, py); // the path must be rasterizable
    }

    // combine two traced paths into all possible subpaths and return pixel contributions
    ...
    PathContribution CombinePaths(const Path& EyePath, const Path& LightPath, const int SpecifiedNumEyeVertices = -1, const int SpecifiedNumLightVertices = -1)
    {
        ...
        for (int PathLength = MinPathLength; PathLength <= MaxPathLength; PathLength++)
        {
            ...
            for (int NumEyeVertices = 0; NumEyeVertices <= PathLength + 1; NumEyeVertices++) // note that num verts = path length + 1
            {
                const int NumLightVertices = (PathLength + 1) - NumEyeVertices;

                if (NumEyeVertices == 0) continue; // no direct hit to the camera (pinhole camera, aperture size = 0)
                if (NumEyeVertices > EyePath.size()) continue; // impossible case
                if (NumLightVertices > LightPath.size()) continue; // impossible case
                ...
                float px = -1.0, py = -1.0; // pixel contribution position in pixel units
                if (!isConnectable(Eyesubpath, Lightsubpath, px, py))
                {
                    continue;
                }

                // construct a full path (Eyesubpath + Lightsubpath -> SampledPath)
...
//                 vec3 ff = SampledPath.eyeEnd().getThroughput();
//                 if (SampledPath.lightSize() == 0)
//                 {
//                     ff *= SampledPath.lightEnd().getThroughput();
//                 }
//                 else
//                 {
//                     ff *= todo_getMaterial(SampledPath.eyeEnd()->getMaterialID()).getBSDF()->evaluate(vec3(), normal, vec3())
//                 }
//                 vec3d f = vec3d(ff);
                ...
                // path mis
                double path_weight = calculateMisWeight(SampledPath);             // double precision is important
//                 assert(path_weight == path_weight);
                ...
                vec3 c = (f * path_weight).toFloat(); // Monte Carlo integration formula
                //////////////////////////////////////////////////////////////////////////
                ...
                Result.c[Result.n] = Contribution(px, py, c, NumLightVertices, NumEyeVertices);
                Result.n++;
    ...
    // accumulate the pixel contributions of a path onto the film (note that normalization for sample numbers is done when writing to image)
    void AccumulatePathContribution(const PathContribution pc)
    {
        ...
        {
            const int ix = int(pc.c[i].x), iy = int(pc.c[i].y); // pixel index, no advanced reconstruction filter is used
    ...
    double calculatePdfA(const std::vector<const Vert*>& vs, const int from_idx, const int next_idx)
    {
        ...
        default:
        case Lens:
            return camera.measurementFunction(next_vertex); // pdfA of sampling the next vertex
            break;
        case Emitter:
        case Diffuse:
            pdf = todo_getMaterial(from_vertex.obj->materialID()).pdfW(vec3(0,0,0), from_vertex.n, normalized_to);
            break;
        case Specular:
//             pdf = 1;
            pdf = from_vertex.pWo;
        ...
        // pdfW to pdfA
        const vec3 next_new_orienting_normal = dot(to, next_vertex.n) < 0 ? next_vertex.n : -next_vertex.n;
        return pdf * (dot(-normalized_to, next_new_orienting_normal) / dot(to, to));
    }
    ...
    double calculateMisWeight(const CombinedPath& path)
    {
        ...
        double PA_y0 = 1.0 / scene.getLightArea();
        double PA_x0 = 1.0;
...
//             return 0;
    ...
    vec3d pathThroughputWeighted(const CombinedPath& path)
    {
        ...
        {
            const Material& mat = todo_getMaterial(path.eye(i).obj->materialID());
            ...
            if (!mat.isLight())
            {
                ...
                bsdf_weight *= 1.0 / path.eye(i).pWo;
            }
            f *= bsdf_weight;
        }
        ...
        {
            const Material& mat = todo_getMaterial(path.light(j).obj->materialID());
            ...
            if (!mat.isLight())
            {
                ...
                bsdf_weight *= 1.0 / path.light(j).pWo;
            }
            f *= bsdf_weight;
        }
        ...
        if (path.lightSize() == 0)
        {
            if (path.eyeSize() > 1)
            {
                const Material& mat_eye_end = todo_getMaterial(path.eyeEnd().obj->materialID());
                vec3 L = vec3(0);
                if (mat_eye_end.isLight())
                {
                    ...
                    {
                        //                     L = mat_eye_end.color() / NUM_PI; // light
        ...
        if (path.lightSize() == 1)
        {
            float pdfA = 1.0 / scene.getLightArea();
            f *= 1.0 / pdfA;
            ...
            if (path.eyeSize() > 1)
            {
                ...
                if (!mat_eye_end.isLight())
                {
                    const vec3 d0 = normalize(path.eye(path.eyeSize() - 2).p - path.eyeEnd().p);
                    const vec3 d1 = normalize(path.lightEnd().p - path.eyeEnd().p);
                    bsdf_eye_end = mat_eye_end.evaluate(d0, path.eyeEnd().n, d1); // includes color
                    bsdf_eye_end *= geometryTerm(path.eyeEnd(), path.lightEnd());
                }
                f *= bsdf_eye_end;

                const Material& mat_light_end = todo_getMaterial(path.lightEnd().obj->materialID());
                vec3 L = vec3(0);
                if (mat_light_end.isLight())
                {
                    ...
                    {
                        //                     L = mat_light_end.color() / NUM_PI; // light
        ...
        if (path.lightSize() > 1)
        {
            ...
            if (!mat_light_end.isLight())
            {
                const vec3 d0 = normalize(path.light(path.lightSize() - 2).p - path.lightEnd().p);
                const vec3 d1 = normalize(path.eyeEnd().p - path.lightEnd().p);
                bsdf_light_end = mat_light_end.evaluate(d0, path.lightEnd().n, d1);
            }
            f *= bsdf_light_end;

            const Material& mat_light = todo_getMaterial(path.light(0).obj->materialID());
            ...
            if (mat_light.isLight())
            {
                ...
                if (dot(dir, path.light(0).n) > 0)
                {
                    ...
                    L = mat_light.color() * NUM_PI; // light (pdfW = cos / pi, pdfA = cos' / r^2 * cos / pi, thus cancelled with G, becomes 1 / pi)
                }
            }
            f *= L;
            float pdfA = 1.0 / scene.getLightArea();
...
public:
    ...
    ReferenceBDPT(const CCamera& camera, FrameBuffer& film, const Scene& scene) :
    ...
    void render(const int MaxEyeEvents)
    {
        // sample the path
        Path eye_path = GenerateEyePath(MaxEyeEvents);
        Path light_path = GenerateLightPath(MaxEyeEvents);
    ...
    void dumpPyramid(float scale)
    {
        ...
        for (int s = 0; s < MaxEvents; s++)
        {
            for (int t = 1; t < MaxEvents; t++)
            {
                ...
                std::string filename = std::string("pyramid/bdpt_s") + str(s) + "_t" + str(t) + ".ppm";
                img.scale(scale);
                img.tonemapGamma(2.2);
